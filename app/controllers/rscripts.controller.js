...
//   console.log('end parse');
  ...
  var rs = db
    ...
    .then((data) => {
      ...
      // console.log("rs ", data);
      res.send({
        message: `rscripts.js Inserted percent_agreement with book=${o.book} study_no=${o.study_no}.`,
      });
    })
    .catch((err) => {
      res.status(500).send({
        message:
          err.message ||
          `rscripts.js Error insert percent_agreement: book: ${o.book} study_no: ${o.study_no}`,
      });
    });
};
...
exports.findAll = (req, res) => {
  ...
  var rs = db
      .query(
	  'select book, study_no, date_created, description, exclude_type, linkage_method, csv_file_name, rscript_file_name from r_script order by book, study_no',
      null
    )
    .then((data) => {
      // console.log("rs ", data);
...
exports.findRScript = (req, res) => {
  ...
  // rscript is in format {book}-{study_no}
  // example 'oba-2'
